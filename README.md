...
node bin/qorm.js    # the CLI (after npm run build)
```
...
```bash
docker run -d --name qorm-pg -e POSTGRES_USER=qorm -e POSTGRES_PASSWORD=qorm \
  -e POSTGRES_DB=qorm -p 5433:5432 postgres
docker run -d --name qorm-mysql -e MYSQL_ROOT_PASSWORD=qorm -e MYSQL_DATABASE=qorm \
  -e MYSQL_USER=qorm -e MYSQL_PASSWORD=qorm -p 3307:3306 mysql:8
npm test
```
...
```bash
qorm makemigrations            # diff models vs. migration state -> migration file
qorm makemigrations --name x   #   (--empty, --dry-run, --check also supported)
qorm migrate                   # apply unapplied migrations
qorm migrate 0002              # migrate to a target (forward or backward)
qorm migrate zero              # unapply everything
qorm migrate --fake            # record without running
qorm migrate --plan            # show what would run
qorm showmigrations            # [X] applied / [ ] pending
qorm squashmigrations          # collapse history into one squashed migration
qorm sqlmigrate 0001           # print a migration's SQL without running it
qorm check                     # validate models & config
qorm flush --yes               # empty all model tables
qorm shell                     # REPL with models, Q/F/aggregates, db preloaded
qorm dbshell                   # open sqlite3 on the configured database
qorm inspectdb                 # generate model code from an existing database
```

Configuration lives in `qorm.config.ts` (or `.js`/`.mjs`):
...
```ts
// migrations/0001_initial.ts — generated by `qorm makemigrations`
export default {
  dependencies: [],
  operations: (ops) => [
    ops.createModel(
...
and state is tracked in a `qorm_migrations` table.
...
src/
  ...
  migrations/
    ...
    recorder.ts       qorm_migrations table
    executor.ts       plan/apply/unapply/--fake/sqlmigrate
  cli.ts              the `qorm` command
bin/qorm.js           CLI launcher
